...
1. **Command**  
   ...
   docker run -it \
     ...
     ai-agent:latest
   ```
2. **Check**: Ensure the container starts, and that `main.py` (or any other entry point) can read these variables.
...
1. **Command**  
   ...
   ```
2. **Check**: The `test_git_clone.py` script should pass if it can clone the public repository. Check the container’s file system to verify the new directory is present.
...
- Add new features (e.g., modifying code in `src/`, running additional tests).
